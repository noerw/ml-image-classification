...
net = mlp.mlp(all_training_data[:,0:3], all_training_data[:,3:],1)
net.mlptrain(all_training_data[:,0:3], all_training_data[:,3:], 0.25, 100)
print(net)
net.confmat(all_training_data[:,0:3], all_training_data[:,3:])
...
for x in range(whole_width):
    for y in range(whole_height):
        if int(prediction[i][0]) == 1:
            classified_pixels[x, y] = (0,0,0)
        elif int(prediction[i][0]) == 2:
            classified_pixels[x, y] = (127, 127, 127)
        else:
            classified_pixels[x, y] = (255, 255, 255)
        i = i + 1
img.show()       
